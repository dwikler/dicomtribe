...
<xsl:stylesheet
    ...
    <xsl:template match="//docbook:table">
        <!--Find DICOM Part 6 Table 8-1. Registry of DICOM Directory Structuring Elements-->
        <xsl:if test="contains(docbook:caption,'Registry of DICOM Directory Structuring Elements')">
            <xsl:element name="DicomDirectoryStructuringElements">
                <xsl:text>&#x0A;</xsl:text>
    ...
    <xsl:template match="docbook:tbody/docbook:tr">
        ...
        <!--Write DICOM directory structure element information in XML according to PS3.19 Native DICOM model schema-->
