...
<xsl:stylesheet
    ...
    <xsl:template match="/">
        <xsl:element name="DicomElements">
            <xsl:text>&#x0A;</xsl:text>
    ...
    <xsl:template match="//docbook:table">
        <!--Find DICOM Part 6 Tables 6-1, 7-1 and 8-1 Elements-->
        <xsl:if test="contains(docbook:caption,'Elements')">
    ...
    <xsl:template match="docbook:tbody/docbook:tr">
        ...
        <!--Write DICOM element information in XML according to PS3.19 Native DICOM model schema-->
