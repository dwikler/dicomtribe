...
<xsl:stylesheet
    ...
    <xsl:template match="//docbook:table">
        <!--Find DICOM Part 6 Table 6-1. Registry of DICOM Data Elements-->
        <xsl:if test="contains(docbook:caption,'Registry of DICOM Data Elements')">
            <xsl:element name="DicomDataElements">
                <xsl:text>&#x0A;</xsl:text>
    ...
    <xsl:template match="docbook:tbody/docbook:tr">
        ...
        <!--Write DICOM data element information in XML according to PS3.19 Native DICOM model schema-->
